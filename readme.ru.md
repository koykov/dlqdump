...
Параллельно работает ограничение по `FlushInterval`. Запоминается момент поступления первого элемента (с начала работы
...
произвольный элемент и пробует его сериализовать в буфер `dst`. Сериализованные данные будут далее перенаправлены в
хранилище.

`dlqdump` из коробки содержит несколько энкодеров:
...
`Restorer` имеет три настройки, согласно которым происходит восстановление данных и отправка в очередь-адресат:
...
сможет перегрузить очередь-адресат восстановленными элементами.
...
тестирования и не имеет смысла в продакшн условиях. А второй обратен энкодеру `marshaller` и способен десериализовать
...
Аналогичным образом, можно написать реализацию `MetricsWriter`-а для иной TSDB, которую вы используете.
